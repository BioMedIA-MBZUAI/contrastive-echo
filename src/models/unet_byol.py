...
from src.models.byolu_model import BYOL

from unet.unet_model import UNet, UNetEncoder
...
class UNetModel(LightningModule):
    ...
    def __init__(
        ...
        byol: dict = {"model_path": "", "batch_size": 256},
        lr: float = 0.001,
        ...
        if self.hparams.byol.model_path != None:
            print("-------UNET BYOL LOADING: " + self.hparams.byol.model_path + "-------")
            pretext_model = BYOL(num_classes=1, input_height=224, batch_size=self.hparams.byol.batch_size, max_epochs=300, num_workers=4)
            pretext_model.load_state_dict(torch.load(self.hparams.byol.model_path)["state_dict"])
            backbone = torchvision.models._utils.IntermediateLayerGetter(pretext_model.online_network.encoder, {'down4':'down4'})
