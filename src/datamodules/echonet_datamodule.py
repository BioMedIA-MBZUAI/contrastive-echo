...
class Echonet(torchvision.datasets.VisionDataset):
    ...
    def __init__(self, root=None, split="train", mean=[0, 0, 0], std=[1, 1, 1]):
        ...
        for image in os.listdir(os.path.join(self.root, "imgs", self.split)):
    ...
    def __getitem__(self, index):
        ...
        img = Image.open(os.path.join(self.root, "imgs", self.split, image)).convert("RGB")
        mask = Image.open(os.path.join(self.root, "masks", self.split, image))
        ...
        mask = transforms.Resize(224, interpolation = torchvision.transforms.functional.InterpolationMode.NEAREST)(mask)
...
class EchonetDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None):
        mean, std = get_mean_and_std(Echonet(root=self.data_dir, split="train"))
        kwargs = {
            "mean": mean,
            "std": std
        }

        self.data_train = Echonet(root=self.data_dir, split="train", **kwargs)
        self.data_val = Echonet(root=self.data_dir, split="val", **kwargs)
        self.data_test = Echonet(root=self.data_dir, split="test", **kwargs)
